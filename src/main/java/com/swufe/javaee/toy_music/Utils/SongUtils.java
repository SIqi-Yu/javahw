...
public class SongUtils {
    ...
    public static List<Song> loadSongs() {
        ...
        try (Stream<String> stream = Files.lines(Paths.get(CONSTANTS.SONG_FILE))) {
            songs = stream.map(line -> {
               ...
               Date date=new Date();
               try{
                   date =new SimpleDateFormat("yyyy-MM-dd").parse(items[4]);
     ...
     */
    public static void addSong(int id, String title, String singer, String image, String description) {
    ...
    public static Song getSongByTitle(String title) {
        List<Song> songs = SongUtils.loadSongs();
        List<Song> results = songs.stream().filter(s -> s.getTitle().equals(title)).collect(Collectors.toList());
        if (results.size() == 0)
            return null;
        else
